...
SELECT s.name , sp.present , CASE 
 WHEN sp.present <= 7 THEN  'poor'
	WHEN sp.present < 15 AND sp.present > 7  THEN  'good'
	WHEN sp.present < 20 AND sp.present > 15  THEN  'better'
	WHEN sp.present > 20 THEN  'Best'
	ELSE 'nodate'
	END 'report'
   FROM student s
INNER join studentpresent sp on
s.studentid = sp.stdid
WHERE sp.month = strftime('%m','now')
...
SELECT s.name, 
        (   SELECT SUM([hour]) 
            ...
            WHERE s.studentid = sh1.stdid AND 
			strftime('%m', datex) = '01'
			) [Working Hours for Jan]  ,
			  (   SELECT SUM([hour]) 
            FROM studenthours sh1
            WHERE s.studentid = sh1.stdid AND 
			strftime('%m', datex) = strftime('%m','2022-02-01')
			) [Working Hours for feb]  
